...
from apps.home.libs.CMSeeK.cmseekdb import basic as cmseek ## Good old module

from apps.home.libs.CMSeeK.VersionDetect import wp as wordpress_version_detect
from apps.home.libs.CMSeeK.deepscans.wp import  userenum as wp_user_enum
from apps.home.libs.CMSeeK.deepscans.wp import  vuln as wp_vuln_scan
from apps.home.libs.CMSeeK.deepscans.wp import  pluginsdetect as wp_plugins_enum
from apps.home.libs.CMSeeK.deepscans.wp import  themedetect as wp_theme_enum
from apps.home.libs.CMSeeK.deepscans.wp import  pathdisc as path_disclosure
from apps.home.libs.CMSeeK.deepscans.wp import  check_reg as check_reg
from apps.home.libs.CMSeeK.cmseekdb import result as sresult
import time
import re
import os

def start(id, url, ua, ga, source, detection_method,test_id):
    '''
    id = ID of the cms
    url = URL of target
    ua = User Agent
                ...
                if vulnss > 0:
                    for i,vuln in enumerate(result['vulnerabilities']):
                        if i == 0 and i != vulnss - 1:
                            ...
                            sresult.end_subsub("Fixed In Version: " + cmseek.bold + cmseek.fgreen + str(vuln['fixed_in']) + cmseek.cln, False, True)
        sresult.end(str(cmseek.total_requests), str(comptime), log_file,test_id)
        return
